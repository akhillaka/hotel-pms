...
export function MobileRoomBoard() {
  ...
  const changeRoomStatus = async (status) => {
    try {
      await axios.patch(\`/api/rooms/\${selectedRoom.id}/status\`, { status });
  ...
  const updateRoom = async (e) => {
    e.preventDefault();
    try {
      await axios.put(\`/api/rooms/\${selectedRoom.id}\`, editRoom);
    ...
    <div style={pageStyle}>
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 8, msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
        {Object.entries(counts).map(([k, v]) => (
          <div key={k} style={{ display: 'flex', alignItems: 'center', background: '#fff', padding: '6px 12px', borderRadius: 20, fontSize: '0.8rem', fontWeight: 600, color: getRoomColor(k), border: \`1px solid \${getRoomColor(k)}30\`, whiteSpace: 'nowrap' }}>
            {k}: {v}
          </div>
        ))}
      </div>
...
export function MobileHousekeeping() {
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.patch(\`/api/housekeeping/tasks/\${id}\`, { status });
  ...
  const updateRemarks = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(\`/api/housekeeping/tasks/\${selectedTask.id}\`, { remarks: editRemarks });
...
export function MobileMoneyManager() {
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    try {
      await axios.post('/api/transactions', { type, amount: Number(amount), category, description: desc });
  ...
  const updateTx = async (e) => {
    e.preventDefault();
    try {
      await axios.put(\`/api/transactions/\${selectedTx.id}\`, { type: editTx.type, amount: Number(editTx.amount), category: editTx.category, description: editTx.description });
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {['Today', 'This Week', 'This Month'].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{
            ...
            background: filter === f ? colors.primary : '#fff',
            color: filter === f ? '#fff' : colors.muted,
            fontWeight: 600, fontSize: '0.85rem', ...tapTargetStyle
          }}>{f}</button>
        ))}
      </div>
          ...
          {txs.map(t => (
            <div key={t.id} onClick={() => { setSelectedTx(t); setEditTx(t); }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} style={{ ...cardStyle, padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 20, background: t.type === 'income' ? colors.green+'15' : colors.red+'15', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {t.type === 'income' ? <TrendingUp size={20} color={colors.green} /> : <TrendingDown size={20} color={colors.red} />}
                </div>
                <div>
              ...
              <span style={{ fontWeight: 800, color: t.type === 'income' ? colors.green : colors.red }}>
...
export function MobileReservations() {
  ...
  const updateRes = async (e) => {
    e.preventDefault();
    try {
      await axios.put(\`/api/reservations/\${selectedRes.id}\`, { guest_name: editRes.guest_name, adults: Number(editRes.adults), children: Number(editRes.children), remarks: editRes.remarks });
...
export function MobileReports() {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const res = await axios.get(\`/api/reports/dashboard?range=\${range}\`);
      setData(res.data);
    } catch (e) {
...
export function MobileAudit() {
  ...
  const fetchLogs = async (p) => {
    setLoading(true);
    try {
      const { data } = await axios.get(\`/api/audit?page=\${p}&limit=20\`);
      if (p === 1) setLogs(data);
          ...
          return (
            <div key={log.id || i} style={{ display: 'flex', gap: 16, padding: '12px 16px', borderBottom: i === logs.length - 1 ? 'none' : '1px solid #f1f5f9' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: 0, fontSize: '0.7rem', color: colors.slate }}>{new Date(log.created_at).toLocaleString()}</p>
              </div>
            </div>
          );
        })}
...
fs.writeFileSync('src/components/MobileViews.jsx', code);
console.log("Successfully wrote to src/components/MobileViews.jsx");
